...
def compute_normalization_constants_dense(
    ...
    max_children = max(len(mutations) for mutations in FUNCTIONAL_CODON_SINGLE_MUTATIONS.values())
    parent_to_children = torch.full((65, max_children), -1, dtype=torch.long, device=device)
    ...
    child_codon_indices = parent_to_children[codon_parents_idxss]  # (N, L_codon, max_children)

    # Create batch and position index tensors for advanced indexing
    batch_indices = torch.arange(N, device=device).view(N, 1, 1).expand(N, L_codon, max_children)
    pos_indices = torch.arange(L_codon, device=device).view(1, L_codon, 1).expand(N, L_codon, max_children)
    ...
    neutral_rates_gathered = neutral_rates_tensor[batch_indices, pos_indices, child_codon_indices_clamped]

    # Convert child codon indices to AA indices using vectorized lookup
    codon_to_aa = torch.full((65,), 20, dtype=torch.long, device=device)  # 20 = invalid/stop
    for codon_idx, aa_idx in AA_IDX_FROM_CODON_IDX.items():
        codon_to_aa[codon_idx] = aa_idx

    child_aa_indices = codon_to_aa[child_codon_indices_clamped]  # (N, L_codon, max_children)
    ...
    selection_factors_gathered = selection_factors[batch_indices, pos_indices, child_aa_indices_clamped]

    # Compute products: neutral_rates * selection_factors
    products = neutral_rates_gathered * selection_factors_gathered  # (N, L_codon, max_children)
    ...
    # Combine all masks
    combined_mask = valid_child_mask & valid_aa_mask & within_range_mask & nonzero_rate_mask
