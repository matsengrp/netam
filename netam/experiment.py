...
class Experiment:
    ...
    def build_model_instances(self, prename):
        model_instances_3 = {
            f"{prename}_cnn_sml": models.CNNModel(kmer_length=3, embedding_dim=6, filter_count=14, kernel_size=7, dropout_rate=0.0),
            f"{prename}_cnn_med": models.CNNModel(kmer_length=3, embedding_dim=9, filter_count=9, kernel_size=11, dropout_rate=0.1),
            f"{prename}_cnn_lrg": models.CNNModel(kmer_length=3, embedding_dim=7, filter_count=19, kernel_size=11, dropout_rate=0.3),
            f"{prename}_cnn_4k": models.CNNModel(kmer_length=3, embedding_dim=12, filter_count=14, kernel_size=17, dropout_rate=0.1),
            f"{prename}_cnn_8k": models.CNNModel(kmer_length=3, embedding_dim=14, filter_count=25, kernel_size=15, dropout_rate=0.0),
            }

        model_instances_5 = {
            f"{prename}_fivemer": models.FivemerModel(kmer_length=5),
            f"{prename}_shmoof": models.SHMoofModel(kmer_length=5, site_count=self.site_count),
        }
        return model_instances_3, model_instances_5

    def data_by_kmer_length_of(self, data_df):
        data_dict = {kmer_length: framework.SHMoofDataset(data_df, kmer_length=kmer_length, site_count=self.site_count) for kmer_length in [1, 3, 5]}
        for data in data_dict.values():
            data.to(self.device)
        return data_dict

    def train_or_load(self, model_name, model, train_dataset, val_dataset, training_params, pretrained_dir="../pretrained"):
    ...
    def build_experiment_df(self, prename, train_data_by_kmer_length, val_data_by_kmer_length, training_params_by_model_name):
        model_instances_3, model_instances_5 = self.build_model_instances(prename)
        experiment_df = \
            pd.concat([ self.experiment_df_of(model_instances_3, train_data_by_kmer_length[3], val_data_by_kmer_length[3]),
                        self.experiment_df_of(model_instances_5, train_data_by_kmer_length[5], val_data_by_kmer_length[5])
                    ], ignore_index=True)
        experiment_df["training_params"] = [training_params_by_model_name.get(row["model_name"], {}) for _, row in experiment_df.iterrows()]
    ...
    def train_experiment_df(self, experiment_df, pretrained_dir="../pretrained"):
        for index, row in experiment_df.iterrows():
            ...
            trained_model = self.train_or_load(model_name, model_instance, train_dataset, val_dataset, training_params, pretrained_dir=pretrained_dir)
            experiment_df.at[index, "model"] = trained_model
            
    def calculate_loss(self, model, dataset):
        model.eval()
        burrito = framework.Burrito(dataset, dataset, model, verbose=False, **self.burrito_params)
